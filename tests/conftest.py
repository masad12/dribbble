...
from testprojectfromattacker import DefaultAioHttpClient, Testprojectfromattacker, AsyncTestprojectfromattacker
from testprojectfromattacker._utils import is_dict
...
logging.getLogger("testprojectfromattacker").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Testprojectfromattacker]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Testprojectfromattacker(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncTestprojectfromattacker]:
